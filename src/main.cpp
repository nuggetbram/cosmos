...
unsigned int nStakeMinAge = 60 * 5; // minimum age for coin age
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal cosmoscoin miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in CosmosCoin
    bool fStrictPayToScriptHash = true; // Always active in CosmosCoin
    ...
    // ppcoin: fees are not collected by miners as in cosmoscoin
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Valentines Day 2015";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("0xee68472e25f7d63649436a30081b82853a18e24a92ec9540dd330069b00ec23d"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("CosmoscoinMiner : proof-of-work not meeting target");

    //// debug print
    printf("CosmoscoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CosmoscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("CosmoscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadCosmoscoinMiner(void* parg);

static bool fGenerateCosmoscoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void CosmoscoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateCosmoscoins || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateCosmoscoins) && !fProofOfStake)
                return;
        }
        ...
        printf("Running CosmoscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadCosmoscoinMiner(void* parg)
{
    ...
    try
    {
        ...
        CosmoscoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadCosmoscoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadCosmoscoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadCosmoscoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateCosmoscoins(bool fGenerate, CWallet* pwallet)
{
    fGenerateCosmoscoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateCosmoscoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d CosmoscoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadCosmoscoinMiner, pwallet))
                printf("Error: NewThread(ThreadCosmoscoinMiner) failed\n");
            Sleep(10);
        }
    }
}
